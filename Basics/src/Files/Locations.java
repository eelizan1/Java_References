...
public class Locations implements Map<Integer, Location> {
    // A map to store the loccations
    private static Map<Integer, Location> locations = new HashMap<Integer, Location>();

    public static void main(String[] args) throws IOException {
        // create file writer object
        // Create file null here because the try block introduces new scope
        FileWriter locFile = null;
        try {
            locFile = new FileWriter("locations.txt");
            // loop through map and get values
            for(Location location : locations.values()) {
                locFile.write(location.getLocationID() + ", " + location.getDescription() + "\n");
            }
            // locFile.close()
        } finally { // use finally block because if there was an error in the locFile.write, it willskip the locFile.close()
